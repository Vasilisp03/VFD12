...
class FutabaVFD {
  public:
    ...
    // Smooth pixel scroll
    void smoothScroll(const char* text, uint16_t speedMs = 40);
  ...
  private:
    ...
    void _getCharColumns(char c, uint8_t cols[5]);
    void _writeCustomChar(uint8_t slot, uint8_t cols[5]);
    void _printRaw(uint8_t pos, uint8_t* charCodes, uint8_t len);
    uint8_t _dinPin;
    uint8_t _clkPin;
    uint8_t _csPin;
    uint8_t _enPin;
